import os
   ...
   def generate_vpn_section():
       """تولید بخش لینک‌های وی‌پی‌ان"""
       ...
       # تولید محتوای بخش وی‌پی‌ان
       vpn_content = f"""## لینک‌های وی‌پی‌ان اختصاصی هر کشور

این بخش شامل لینک‌های خام برای فایل‌های کانکشن وی‌پی‌ان است که هر کدام به یک کشور خاص اختصاص دارند. این فایل‌ها هر ۶ ساعت به‌صورت خودکار به‌روزرسانی می‌شوند.
...
"""
       ...
       for country in sorted_data:
           vpn_content += f"| {country['name']} | {country['connections']} | [{country['file']}]({BASE_URL}/{country['file']}) |\n"
...
"""
   ...
   def update_readme():
       """به‌روزرسانی README.md با حفظ محتوای موجود"""
       vpn_section_start = "## لینک‌های وی‌پی‌ان اختصاصی هر کشور"
       vpn_section_end = "##"  # فرض می‌کنیم بخش بعدی با یه هدر شروع می‌شه

       # خواندن محتوای موجود README.md
       try:
       ...
       # تولید بخش جدید وی‌پی‌ان
       new_vpn_section = generate_vpn_section()

       # جایگزینی بخش وی‌پی‌ان یا اضافه کردن به انتها
       if vpn_section_start in current_content:
           # پیدا کردن بخش موجود و جایگزینی
           pattern = f"{vpn_section_start}.*?(?={vpn_section_end}|$)"
           updated_content = re.sub(pattern, new_vpn_section, current_content, flags=re.DOTALL)
       else:
           # اضافه کردن بخش جدید به انتهای فایل
